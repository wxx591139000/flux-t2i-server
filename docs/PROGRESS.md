...
### 2026-09-24 追加 · P0-5：线上实测暴露的阻塞问题（已修）

**触发**：用户重启 9620 后给图图发"你好"，长时间无回复。查日志定位到 **P0-5**（比 P0-1~4 更贴近"用户当场遇到"）。

| 编号 | 缺陷 | 修法 |
|---|---|---|
| **P0-5** ★ | **WS 回调里同步做耗时工作**：lark 在 asyncio 事件循环里**同步**调用 `_on_message`，而 `submit()` 含中文翻译（实测 68 秒）→ 心跳发不出去 → 飞书 **3003 ping timeout 踢连接** + **事件重投** | 回调只做去重/过滤/入队（毫秒级），2 个工作线程消费；提交前先发**即时回执** |
| P0-6 | `notified` **无条件置位**（未判断发送结果）→ 首次发送失败则用户**永远**收不到那条通知 | 只在 `send_direct` 返回 True 时置位；失败下轮重试 |
| P0-7 | 提交前**无任何反馈**，用户要干等几十秒 | 先回「⏳ 收到！正在解析提示词…」，再回任务号 |

**门扩展**：21 项 → **26 项**（新增"WS 回调不阻塞""入队最终被处理""即时回执""notified 只在成功时置位"）。
**全量门禁 19/19 通过**；变异测试：改回同步调用 → 门报 `2519 ms` 变红。

**同时确认的线上事实（不是缺陷，但要告知）**
- 图图**有发送能力**：用它的应用直接调 `im/v1/messages` 给 owner 发消息 → `code:0, msg:success`
- 但当前**没有任何 GPU 机在线** → 任务进 `waiting`（`[SERVER_DOWN] 无可用 flux 服务器`），
  **即使链路全通也出不了图**。任务会在机器上线后自动恢复（`_recover_waiting_tasks`）
- 那条"你好"任务 `3ada124e5f334de7` 就停在 `waiting`，是"没出图"的直接原因

**版本**：v2.9.1 → **v2.9.2**

### 2026-09-24 追加 · 图图 P1 第一块：取消任务（v2.10.0）

需求原话「像小白一样有取消任务的能力吗」。结论：**底层早有，缺的是命令面**。

**已完成**
- 图图命令面：`帮助` / `我的任务`（裸 `取消` 也看列表）/ `取消 01` / `取消 <hex 短码>`
- 顺序硬要求：命令识别早于提示词提交（否则命令被当提示词 → **扣额度出废图**）
- 严格判据：只认"取消 + 序号/短码"，「取消一切杂乱背景」这类提示词**不被吞**
- ★ 修真缺陷：已删 `waiting` 任务会在内存等待池恢复时复活，并在 4 小时超时分支**拿到退款**
  （= 把「删了重传刷额度」开了口子）。修法：`pooled` 判据 + 恢复入口墓碑过滤（三道闸）
- 跨渠道：网页端删任务后，飞书轮询靠墓碑终止跟踪（否则永不收敛）

**门**
- 新增 `tests/test_feishu_bot_cancel.py`（55 项）
- `tests/test_delete_job.py` → 9 条不变量（新增 T11，10 项，含阳性对照）
- 全量 **20/20**；变异 **7/7** 被抓住

**下一步（P1 剩余）**
- 参数状态机（`/size /n /model`，纯文本）与 `/bind` 激活码绑定
  （决策 2 已把 `/bind` 从 P5 提到 P1）
